...
#new 9 month salary + summer any summer salary you get (calculated as 1/9 of total)
...
Sal.1.9.with.summer=Sal.1.9+(Sal.init.12/9*salary.grant.months.yr1) 
Sal.2.9.with.summer=Sal.1.9+(Sal.init.12/9*salary.grant.months.yr2) 
Sal.3.9.with.summer=Sal.1.9+(Sal.init.12/9*salary.grant.months.yr3) 

annual.rate=6 #annual percent interest rate of investments (estimate)
years_to_project<-20  #for projections of future salary
prob=0.50 #prob of having summer salary in years 4-24
actual_summer_salary_funded<-1 #You can estimate how many months of summer salary ytou get 

## retain 12 month appointment
#############
...
# YEAR 1
P1.9=(Sal.1.9/12)*0.0514 #amount of monthly deposit = monthly salary * percent placed in investment account
...
# YEAR 2
P2.9=(Sal.2.9/12)*0.0514 #amount of monthly deposit = monthly salary * percent placed in investment account
...
# YEAR 3
P3.9=(Sal.3.9/12)*0.0514 #amount of monthly deposit = monthly salary * percent placed in investment account
...
Sal.init.12 #salary at end of 3 years on 12 mo appt
# 9 Month
...
# shift to 9 month assume never get another summer after 3rd year
...
investment_return <- function(x,y,z) {
  ...
  for (i in 1:z) {
    ...
    summer_salary<-Base_Salary/12*rbinom(length(prob), size = 1, prob=prob) #QUESTION THIS SHOULD BE 9 or 12??!!!
...
# 
# SI+OMETHING WROING HERE SO REDID ABIVE
# # stay at 12 month
# Base_Salary<-Sal.3.12
# Total.invest<-Invest.proj.3.12
# 
# 
# 
# investment_return <- function(x,y,z) {
...
#   for (i in 1:z) {
#     
#     prob=0 #prob of having summer salary in years 4-24
#     summer_salary<-Base_Salary/12*rbinom(length(prob), size = 1, prob=prob)
#     actual_summer_salary_funded<-0
#     total_salary<-Base_Salary+(summer_salary*actual_summer_salary_funded)
#     data$total_salary[i]<-total_salary
#     Projections=((total_salary)/12)*0.0514 #amount of monthly deposit = monthly salary * percent placed in investment account
#     prin.proj <- Projections*(1+rate/100)^(0:(n-1))
#     
#     carry.forward<-Total.invest*(1+rate/100)^(0:(n-1))
#     carry.forward<-carry.forward[12]
#     int.proj  <- prin.proj * rate/100
#     totalInt.proj <- sum(int.proj)
#     Invest.proj<-sum(prin.proj)+carry.forward+totalInt.proj #Principal+Interest
...
# 
# 
# projections.12mos<-investment_return(Base_Salary,Total.invest,years_to_project)
# projections.12mos
